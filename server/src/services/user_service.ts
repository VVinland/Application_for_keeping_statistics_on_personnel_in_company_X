...
class UserService {
    ...
    async registration(firstname: string, lastname: string, middlename: string, dateOfBirthday: Date, role: string, salaryAmount: number,
        dateHired: Date): Promise<string> {

        const link_registering: string = v4();
        const newEmployee = await db.query(`INSERT INTO EMPLOYEE(firstname, lastname, middlename,
            date_of_birth, role, salary_amount, hire_day) values($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
            [firstname, lastname, middlename, dateOfBirthday, role, salaryAmount, dateHired]);
    ...
    async lastStepOfRegisteringEmployee(email: string, password: string, link_registering: string): Promise<UserDtoAndTokens> {
        let candidate = await db.query(`SELECT * FROM LAST_STEP_OF_REGISTERING_EMPLOYEE WHERE email=$1`,
            [email]);
        ...
        candidate = await db.query(`UPDATE LAST_STEP_OF_REGISTERING_EMPLOYEE SET password=$1, email=$2
                WHERE link_registering=$3 RETURNING user_id`,
            [hashPassword, email, link_registering]);
    ...
    async login(email: string, password: string): Promise<UserDtoAndTokens> {
        let candidate = await db.query(`SELECT * FROM LAST_STEP_OF_REGISTERING_EMPLOYEE WHERE email=$1`,
            [email]);
    ...
    async refresh(refreshToken: string): Promise<UserDtoAndTokens> {

        if (!refreshToken) {
            throw ApiError.UserNotAuthorized();
        }

        const employeeData = tokenService.validateRefreshToken(refreshToken) as EmployeeData;
    ...
    async getAllUsers() {
        const employees: QueryResult<EmployeeData> = await db.query(`SELECT * FROM EMPLOYEE`);
        return employees.rows;
    }

    async updateEmployee(id: number, firstname: string, lastname: string, middlename: string, dateOfBirthday: Date, role: string, salaryAmount: number,
        dateHired: Date) {
        const updatedEmployee: QueryResult<EmployeeData> = await db.query(`UPDATE EMPLOYEE SET firstname=$1, lastname=$2, middlename=$3, date_of_birth=$4, role=$5,
    ...
    async getNumberHiredEmployees(value: string): Promise<number> {
        switch (value) {
    ...
    async getBirthdays(value: string) {
        switch (value) {
            case 'For the current month': {
                const birthdays: QueryResult<EmployeeData> = await db.query(
            ...
            default: {
                return [] as EmployeeData[];
            }
        }
    }

    async deleteEmployee(id: number, dismissalDay: Date) {
        ...
        const firedEmployeesData: QueryResult<FiredEmployeeData> = await db.query(`INSERT INTO FIRED_EMPLOYEES(firstname, lastname, middlename,
    ...
    async getNumberFiredEmployees(value: string) {
        switch (value) {
            case 'For the current month': {
    ...
    async getSalaries(value: string) {
        switch (value) {
            case 'For the current year': {
...
function getSalariesResult(employees: Salaries[], firedEmployees: Salaries[]) {
